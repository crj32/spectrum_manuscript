...
miRNAseq <- read.csv('PCPG.mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.data.txt',sep='\t')
...
RNAseq <- read.csv('PCPG.rnaseqv2__illuminahiseq_rnaseqv2__unc_edu__Level_3__RSEM_genes_normalized__data.data.txt',sep='\t')
...
protein <- read.csv('PCPG.protein_exp__mda_rppa_core__mdanderson_org__Level_3__protein_normalization__data.data.txt',sep='\t')
protein <- protein[-1,]
...
clinical <- read.csv('PCPG.clin.merged.picked.txt',sep='\t')
...
pcpg <- list(RNAseq=RNAseq,miRNAseq=miRNAseq,protein=protein,clinical=clinical)
rm(list=setdiff(ls(), "pcpg"))
