...
miRNAseq <- read.csv('LGG.mirnaseq__illuminahiseq_mirnaseq__bcgsc_ca__Level_3__miR_gene_expression__data.data.txt',sep='\t')
...
RNAseq <- read.csv('LGG.rnaseqv2__illuminahiseq_rnaseqv2__unc_edu__Level_3__RSEM_genes_normalized__data.data.txt',sep='\t')
...
protein <- read.csv('LGG.protein_exp__mda_rppa_core__mdanderson_org__Level_3__protein_normalization__data.data.txt',sep='\t')
protein <- protein[-1,]
...
clinical <- read.csv('LGG.clin.merged.picked.txt',sep='\t')
...
brainlower <- list(RNAseq=RNAseq,miRNAseq=miRNAseq,protein=protein,clinical=clinical)
rm(list=setdiff(ls(), "brainlower"))
